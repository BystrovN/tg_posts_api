...
@router.post('/', response=PostOutSchema, url_name='post-create')
...
@router.patch('/{post_id}/', response=PostOutSchema, url_name='post-update')
def update_post(request, post_id: int, data: PatchDict[PostInSchema]):
    return crud.update_post(post_id, data)


@router.delete('/{post_id}/', response={204: None, 404: dict}, url_name='post-delete')
